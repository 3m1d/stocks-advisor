...
class FeatureGenerator:
    ...
    def _aggregate_to_2h(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Aggregate hourly data to 2-hour intervals for main trading session only.

        Args:
            df: DataFrame with columns ['begin', 'open', 'high', 'low', 'close', 'volume', 'value']
            Must be sorted by 'begin'

        Returns:
            Aggregated DataFrame with 2-hour intervals
        """
        if df.empty:
            return df
        ...
        df = df[
            (df['begin'].dt.time >= pd.Timestamp('10:00:00').time())
    ...
    def generate(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # ===== 7. VOLUME =====
        df_processed['VOLUME_RATIO_20'] = df_processed['volume'] / df_processed['volume'].rolling(window=20).mean()
        df_processed['VOLUME_RATIO_50'] = df_processed['volume'] / df_processed['volume'].rolling(window=50).mean()
        df_processed['VOLUME_RATIO_200'] = df_processed['volume'] / df_processed['volume'].rolling(window=200).mean()
    ...
    def add_targets(self, df: pd.DataFrame, horizon_days: int = 7) -> pd.DataFrame:
        ...
        df_sorted['target_price_change'] = (df_sorted['future_price'] / df_sorted['close'] - 1) * 100
    ...
    def process(
        ...
        Args:
            ...
            aggregate_2h: Aggregate data to 2-hour intervals before feature generation
        ...
        # Aggregate to 2-hour intervals
        if aggregate_2h:
            df = self._aggregate_to_2h(df)
