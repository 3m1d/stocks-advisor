...
def run_async_safe(coro: Coroutine[Any, Any, T]) -> T:
    """Run async code from sync context (works in Jupyter with a running loop)."""
    try:
...
def load_features_for_ticker(ticker: str) -> pd.DataFrame:
    """Sync entry: load DB, build features for one ticker (Jupyter-safe)."""
    features_df, _ = run_async_safe(load_features_bundle(ticker))
    return features_df


def load_features_and_enrichments(ticker: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Sync entry: features + raw enrichments for error analysis."""
