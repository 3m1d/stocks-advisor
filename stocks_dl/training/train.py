...
def plot_metrics(train_losses: List[float], val_losses: List[float], val_metrics: Dict[str, List[float]]):
...
def training_epoch(
    ...
    *,
    ...
    model.train()
    for features, targets, lengths in tqdm(loader, desc=tqdm_desc, disable=not verbose):
...
def train(
    ...
    verbose=False: без tqdm и без построчного лога эпох.
    show_plots=True: графики и подробный вывод каждой эпохи.
    """
    train_losses, val_losses = [], []
    val_metrics = {'mae': [], 'rmse': [], 'r2': [], 'direction_accuracy': []}
    criterion = nn.MSELoss()
    log_epochs = show_plots or verbose
    ...
    if not show_plots and not verbose and num_epochs > 0:
        print(
            f'Trained {num_epochs} epochs: train={train_losses[-1]:.6f} val={val_losses[-1]:.6f} '
            f'mae={val_metrics["mae"][-1]:.4f}% dir_acc={val_metrics["direction_accuracy"][-1]:.4f}'
        )
